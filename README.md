...
**Windows 10/11 için oyun pingini etkilemeyen, minimum kaynak tüketimli ve akıllı tünellemeli Discord RTC & Ses Engeli Kaldırma Aracı.**

 **TR** Türkçe  | **EN** [English](README_EN.md)
...
1. **İndir:** 👉 **[v1.0 Sürümünü İndir (.zip)](https://github.com/abdullatifaslan/discord-adaptive-bypass/releases/download/v1.0/discord-adaptive-bypass-main.zip)**
2. **Ayıkla:** `.zip` arşivini bir klasöre çıkartın.
3. **Çalıştır:** Klasör içindeki **`start.bat`** dosyasına sağ tıklayıp **Yönetici Olarak Çalıştır**'a basın.

> 💡 **İşiniz bittiğinde pencereyi kapatmanız yeterlidir.** Arka plandaki bağımsız bekçi protokolü tüm sürücüleri ve servisleri anında temizler.
...
## 🛡️ Güvenlik & Antivirüs Uyarısı (False Positive)

> [!NOTE]
> **Antivirüs / Windows Defender Uyarısı Hakkında**  
> Araç, düşük seviyeli paket tünellemesi yapabilmek için `WinDivert` çekirdek sürücüsünü kullanır. Henüz yeni yayınlanan açık kaynaklı bir proje olduğu için Windows Defender veya tarayıcınız yanlış alarm (**False Positive**) verebilir.
> 
...
> * **Şeffaflık:** Proje %100 açık kaynaklıdır. Dileyen herkes klasör içindeki `.ps1` ve `.bat` kodlarını çalıştırılmadan önce inceleyebilir.
...
Farklı bir İnternet Servis Sağlayıcı (ISP) kullanıyorsanız ve yeni bir çalışma metodu bulduysanız ya da bir hata ile karşılaştıysanız **[Issues](https://github.com/abdullatifaslan/discord-adaptive-bypass/issues)** sekmesinden bildirimde bulunabilirsiniz.

Proje işinize yaradıysa GitHub üzerinden bir **Star (⭐)** vererek daha fazla kişiye ulaşmasına katkı sağlayabilirsiniz!
